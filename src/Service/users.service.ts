...
export class UsersService {
  constructor(private readonly _cacheService: CacheManagerService) {}
  ...
  async createUser(newUser: CreateUserRequest): Promise<SucessfulResponse> {
    ...
    const existingProfile: Profile = this._profileValidation(
      profiles,
      newUser.profile,
    );
    ...
    let users: User[] =
      (await this._cacheService.get<User[]>('allUsers')) || [];
  ...
  async updateUserById(body: UpdateUserRequest): Promise<SucessfulResponse> {
    ...
    const profiles: Profile[] =
      await this._cacheService.get<any[]>('defaultProfiles');
    const existingProfile: Profile = this._profileValidation(
      profiles,
      body.profile,
    );
  ...
  public _profileValidation(profiles: any[], profileCode: string): any {
    ...
    if (!profile) {
      throw new HttpCustomException(
        `Perfil con código ${profileCode} no encontrado.`,
        StatusCodeEnums.PROFILE_NOT_FOUNT,
      );
    }
    return profile;
  }
